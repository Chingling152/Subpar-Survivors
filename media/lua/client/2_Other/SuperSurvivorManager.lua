    ...
    if (ID ~= nil) and (square ~= nil) then -- 
        ...
        if (melewep ~= nil) then
            ...
            if not self.SuperSurvivors[ID].LastMeleUsed then
                self.SuperSurvivors[ID].LastMeleUsed = self.SuperSurvivors[ID]:getBag():FindAndReturn(melewep)
            end
        end
        if (gunwep ~= nil) then
        ...
        elseif (self.SuperSurvivors[ID]:getAIMode() == "Guard") then
            ...
            if self.SuperSurvivors[ID]:getGroup() ~= nil then
                ...
                if (area) then
                    self.SuperSurvivors[ID]:getTaskManager():AddToTop(
                        WanderInAreaTask:new(self.SuperSurvivors[ID], area))
                    self.SuperSurvivors[ID]:getTaskManager():setTaskUpdateLimit(10)
                else
                    self.SuperSurvivors[ID]:getTaskManager():AddToTop(
                        GuardTask:new(self.SuperSurvivors[ID], self.SuperSurvivors[ID].player:getCurrentSquare()))
                end
            end
...
function SuperSurvivorManager:update()
    ...
    for i = 1, self.SurvivorCount + 1 do
        if (self.SuperSurvivors[i] ~= nil and self.MainPlayer ~= i) then
            if (self.SuperSurvivors[i].TargetSquare ~= nil and self.SuperSurvivors[i].TargetSquare:getZ() ~=
                self.SuperSurvivors[i].player:getZ() and getGameSpeed() > 1) then
                print("DANGER ZONE")
        ...
        if (self.SuperSurvivors[i] ~= nil) and (self.MainPlayer ~= i) and (self.SuperSurvivors[i]:updateTime()) and
            (not self.SuperSurvivors[i].player:isAsleep()) and (self.SuperSurvivors[i]:isInCell()) then
            self.SuperSurvivors[i]:update()
            if (self.SuperSurvivors[i].DebugMode) then
                print(self.SuperSurvivors[i]:getName() .. " update()")
            end
        end
    end
            ...
            if (distance < maxdistance) and (self.SuperSurvivors[i]:Get():CanSee(SSV:Get())) then
                ...
                if (not self.SuperSurvivors[i]:isInGroup(SSW:Get()) and not self.SuperSurvivors[i]:isInGroup(SSV:Get())) then
                    ...
                    if (self.SuperSurvivors[i]:usingGun()) and (ZombRand(2) == 1) then
                        ...
                        self.SuperSurvivors[i]:getTaskManager():AddToTop(
                            FleeFromHereTask:new(self.SuperSurvivors[i], SSW:Get():getCurrentSquare()))
                    end
...
function SuperSurvivorManager:GunShotHandle(SSW)
    ...
    for i = 1, self.SurvivorCount + 1 do
        if (self.SuperSurvivors[i] ~= nil) and (self.SuperSurvivors[i]:isInCell()) then
            ...
            if (self.SuperSurvivors[i].player:getModData().surender) and (distance < maxdistance) and
                self.SuperSurvivors[i]:Get():CanSee(SSW:Get()) and
                self.SuperSurvivors[i].player:CanSee(getSpecificPlayer(0)) then
                -- flee from the crazy murderer
                print(self.SuperSurvivors[i]:getName() .. " fleeing from the crazy murderer")
            ...
            if (self.SuperSurvivors[i].player:getModData().isHostile or self.SuperSurvivors[i]:getCurrentTask() ==
                "Guard" or self.SuperSurvivors[i]:getCurrentTask() == "Patrol") and
                self.SuperSurvivors[i]:getTaskManager():getCurrentTask() ~= "Surender" and
                not self.SuperSurvivors[i].player:isDead() and
                not self.SuperSurvivors[i]:RealCanSee(getSpecificPlayer(0)) and
                (getDistanceBetween(getSpecificPlayer(0), self.SuperSurvivors[i].player) <= range) then
                print(self.SuperSurvivors[i]:getName() .. " adding go check it out task")
                self.SuperSurvivors[i]:getTaskManager():AddToTop(
                    GoCheckItOutTask:new(self.SuperSurvivors[i], getSpecificPlayer(0):getCurrentSquare()))
            end
